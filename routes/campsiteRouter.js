...
.options(cors.corsWithOptions, (req, res) => res.sendStatus(200))
.get(cors.cors,(req, res, next) => { //allows us to store whatever the client sends as part of the path after the slash as a route parameter named campsiteId
    ...
    Campsite.findById(req.params.campsiteId)
        .then(campsite => {
            if (campsite.comments.id(req.params.commentId).author._id.equals(req.user._id)) {
                ...
                err = new Error(`Not author of ${req.params.campsiteId}`);
                err.status = 403;
                return next(err);
            }
        })
